...
class Pose:
    ...
    def __init__(self, trans_xyz, quat_xyzw):
        self.x = trans_xyz[0]
        self.y = trans_xyz[1]
        self.z = trans_xyz[2]
        # quat_xyzw = [quat_xyzw[3], -quat_xyzw[2], quat_xyzw[0], -quat_xyzw[1]]
        self.qw = quat_xyzw[3]
        self.qx = quat_xyzw[0]
        self.qy = quat_xyzw[1]
        self.qz = quat_xyzw[2]
        # print(quat_xyzw)
        self.roll, self.pitch, self.yaw = self.__to_angles(*quat_xyzw)
        self.reach_position = False


    @staticmethod
    def __to_angles(w, x, y, z):
        ysqr = y * y
        # logging.info(f"Qauternions: {[w,x,y,z]}")
        t0 = +2.0 * (w * x + y * z)
        t1 = +1.0 - 2.0 * (x * x + ysqr)
...
class NatNetClient:
    ...
    def get_state(self, body_id=0):
        for rigid_body in self.rigidBodyList:
            if rigid_body[0] == body_id and rigid_body[3]:
                translation = rigid_body[1]
                quaternion = rigid_body[2]
